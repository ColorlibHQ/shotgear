...
function shotgear_pagination( $args = array() ) {
    ...
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'shotgear_pagination_defaults', $defaults )
    );
    ...
    if ( $previous && (1 != $page) )
        $echo .= '<li class="previous pgn__prev"><a href="' . esc_attr( $previous ) . '" title="' . esc_html__( 'previous', 'shotgear' ) . '">' . esc_html__( 'Previous', 'shotgear' ) . '</a></li>';
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="next pgn__next"><a href="' . esc_attr( $next ) . '" title="' . esc_html__( 'next', 'shotgear') . '">' . esc_html__( 'Next', 'shotgear') . '</a></li>';
