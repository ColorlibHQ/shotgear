...
add_action( 'wp_enqueue_scripts', 'shotgear_sl_enqueue_scripts' );
function shotgear_sl_enqueue_scripts() {

	wp_enqueue_script( 'shotgear-post-likes', SHOTGEAR_DIR_JS_URI . 'post-likes.js', array( 'shotgear-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_shotgear_process_simple_like', 'shotgear_process_simple_like' );
add_action( 'wp_ajax_shotgear_process_simple_like', 'shotgear_process_simple_like' );
function shotgear_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'shotgear' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'shotgear' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'shotgear' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'shotgear' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'shotgear' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'shotgear' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'shotgear_show_user_likes' );
add_action( 'edit_user_profile', 'shotgear_show_user_likes' );
function shotgear_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'shotgear' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'shotgear' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
